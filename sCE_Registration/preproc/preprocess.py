...
class ImgConv:
    ...
    def load_subject(self, subject_ID: str, HU_min: int = None, HU_max: int = None, transform_flag: bool = False) -> list:
        ...
        for i in range(len(image_names)):
            ...
            if img.GetSpacing()[2] != 1.0:
                print(f"{image_names[i]}: {img.GetSpacing()}")
                continue
    ...
    def save_data(
        ...
        for subject in subjects:
            if not os.path.exists(f"{self.save_path}/Images/{subject}"):
                os.mkdir(f"{self.save_path}/Images/{subject}")
            if not os.path.exists(f"{self.save_path}/Segmentations/{subject}"):
                os.mkdir(f"{self.save_path}/Segmentations/{subject}")
            proc = self.load_subject(subject, HU_min=HU_min, HU_max=HU_max)
            ...
            for name, img in zip(img_names, img_arrays):
                ...
                if subvol_depth > 0:
                    ...
                    for i in range(0, vol_thick, self.output_dims[2]):
                        ...
                        np.save(f"{self.save_path}/Images/{subject}/{stem}_{idx:03d}.npy", sub_vol)
                        idx += 1
                        count += 1

                else:
                    if file_type == "npy":
                        np.save(f"{self.save_path}/Images/{subject}/{stem}.npy", img[::down_sample, ::down_sample, :])
                    
                    else:
                        assert down_sample == 1, "Down-sample > 1 only with npy"
                        itk.WriteImage(img, f"{self.save_path}/Images/{subject}/{stem}.nii")
                ...
                for name, seg in zip(seg_names, seg_arrays):
                    ...
                    if subvol_depth > 0:
                        ...
                        for i in range(0, vol_thick, self.output_dims[2]):
                            ...
                            np.save(f"{self.save_path}/Segmentations/{subject}/{stem}_{idx:03d}.npy", sub_vol)
                            idx += 1
                            count += 1

                    else:
                        if file_type == "npy":
                            np.save(f"{self.save_path}/Segmentations/{subject}/{stem}.npy", seg[::down_sample, ::down_sample, :])

                        else:
                            assert down_sample == 1, "Down-sample > 1 only with npy"
                            itk.WriteImage(seg, f"{self.save_path}/Segmentations/{subject}/{stem}.nii")
    ...
    def check_saved(save_path):
        ...
        for img_name in subjects:
            subject_imgs = []
            path = f"{save_path}/Images/{img_name}"
            ...
            path = f"{save_path}/Segmentations/{img_name}"
...
if __name__ == "__main__":
    ...
    SAVE_PATH = "D:/ProjectImages/SyntheticContrastReg"

    to_include = None#["T002A1", "T004A0"]

    with open("sce-Registration/preproc/ignore.json", 'r') as fp:
        ignore = json.load(fp)

    subject_ignore = list(ignore["subject_ignore"].keys())
    image_ignore = None #ignore["image_ignore"]
    ...
    Test.list_images(
        ...
        ).save_data(HU_min=-500, HU_max=2500, file_type="npy")

    Test.check_processed_imgs(SAVE_PATH)
